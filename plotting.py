...
def grapher(k):    
    ...
    for i in range(width/k):
        ...
        numbers=font1.render(""+str(txtx),1,black)
        numbersy=font1.render(""+str(txty*-1),1,black)        
        txtx=txtx+1;
        txty=(txty+1);
...
def mymain(n,eq):
    ...
    heading=font.render("Graph Utility Tool",1,(blue));
    screen.blit(heading,(width+100,10));
    heading=font.render("Enter any equation of form Y=F(x)",1,(black));
    screen.blit(heading,(width+10,40));
    heading=font.render("To use Trignometric Function use S,C,T",1,(black));
    screen.blit(heading,(width+10,130));
    heading=font.render("To use Log Function use L,N",1,(black));
    screen.blit(heading,(width+10,70));
    heading=font.render("To use Root or Abs use R,A",1,(black));
    screen.blit(heading,(width+10,100));
    screen.set_clip(None);
    equation=[]
    done=0;
    running=1;
...
#Instructions and Results                  
    ...
    while running:
        ...
        heading=font.render("To add Graph press G",1,(black));
        screen.blit(heading,(width+10,100));
        heading=font.render("To find point of Intersection press I",1,(black));
        screen.blit(heading,(width+10,130));
        heading=font.render("To find X and Y Intercept press X,Y",1,(black));
        screen.blit(heading,(width+10,160));                                
        
        xdisplay=string.join(xValue)
        plotx=font.render("x = "+str(xdisplay),1,(black));
        screen.blit(plotx,(width+10,200));
        
        ploty=font.render("("+str(xVal)+","+str(yVal)+")",1,(black))                
        screen.blit(ploty,(width+220,200));
        screen.set_clip(None)
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                    ...
                    for i in range(width):
                        try:                            
                            ...
                            if y1==y2:
                                ...
                                heading=font.render("Point of Interaction is ("+str(x)+","+str(y1)+")",1,(black));
                                screen.blit(heading,(width+100,480));
                                pygame.draw.circle(screen,red,(int(width/2+x*k),int(height/2-y1*k)),8,2)#highlighting the Y intercept                                                                
                                break
                            #print y
                            
                        except:
                                heading=font.render("Point of Interaction DNE",1,(black));
                                screen.blit(heading,(width+100,480));  
                elif event.key == pygame.K_s:
                    k=25
                    grapher(k)
